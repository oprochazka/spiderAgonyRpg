...
(define my_first_tree (lambda()

		(let* ((h_scale 184) ;výška vysledného obrázku
			...
			(my-list (list 1 2)) ; obrázky ze souboru				
				...
				(left 286) ;Pozice začatku vystřihu zleva v originálu
				(top 1); Pozice začátku výstřihu shora v originálu
				(height_frame 538) ; výška obrázku v originále
				(width_frame 387)	;šířka obrázku v originále
					(sides 4) ; počet stran natočení

					 ;; POstava cut 
					(w_scale (round (* h_scale (/ width_frame height_frame))))
...
(define my_func )
 (script-fu-register
          "my_first_tree"                        ;func name
          "skript_tree"                                  ;menu label
        ...
        (script-fu-menu-register "my_first_tree" "<Toolbox>/Xtns/Script-Fu/Text")
